...
impl FilterContext {
    ...
    pub fn set_bytes<T: AsRef<[u8]>>(
        ...
        let _ = self.set(
            field,
            LiteralValue::Bytes(Arc::new(value.as_ref().to_vec())),
            schema,
        );
        self
    }
...
mod tests {
    ...
    fn test_context_builder_and_typed_setters() {
        ...
        assert_eq!(
            ctx.get_array("arr", &sch),
            Some(Arc::new(vec![LiteralValue::Int(1), LiteralValue::Int(2)]))
        );
    }
    ...
    fn test_type_checking() {
        ...
        let res = ctx.set(
            "foo",
            LiteralValue::Bytes(Arc::new(b"not an int".to_vec())),
            &sch,
        );
        assert!(res.is_err());
        // Correct type
    ...
    fn test_array_type_checking() {
        ...
        let arr = LiteralValue::Array(Arc::new(vec![LiteralValue::Bytes(Arc::new(
            b"bad".to_vec(),
        ))]));
    ...
    fn test_serialization_deserialization() {
        ...
        ctx.set("bar", LiteralValue::Bytes(Arc::new(b"abc".to_vec())), &sch)
            .unwrap();
